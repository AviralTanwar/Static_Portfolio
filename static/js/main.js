...
async function openResumeModal(e) {
  ...
  const container = modal.querySelector('.rm-options');

  if (!container.hasChildNodes()) {
    try {
      const r = await fetch('static/data/resumes.json');
      const resumes = await r.json();
      container.innerHTML = Object.entries(resumes).map(([role, entry]) => `
        <button class="rm-opt" data-role="${role}">
          <span class="rm-opt-tag">${entry.tag}</span>
          <span class="rm-opt-label">${entry.label}</span>
          <span class="rm-opt-desc">${entry.desc}</span>
        </button>`).join('');
      container.querySelectorAll('.rm-opt').forEach(btn => {
        btn.addEventListener('click', () => pickRole(btn.dataset.role));
      });
    } catch {
      container.innerHTML = '<p style="color:var(--accent);font-size:11px;">Could not load options.</p>';
    }
  }
...
<body>
  ...
  <button class="cl-print-btn" onclick="window.print()">↓ Save as PDF</button>
</body>
</html>`;
